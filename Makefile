...
ifeq ($(UNAME),Darwin)
  ...
  ifeq ($(CPU),arm) # iPhone/iPod Touch
    ...
    #-ftree-vectorizer-verbose not supported by clang
...
darwin:
	...
	-rm -f -- $(SOURCES:.c=.o) $(SOURCES_LIB:.c=.o) $(SHARED_SOURCE:.c=.o)
	-rm -f -- $(LIBRARY_NAME).o
	$$(for f in *.pd_darwin; do mv "$$f" "$${f/_tilde./~.}"; done)
	mkdir -p pd_darwin
	mv *.pd_darwin pd_darwin/
# darwin leaves the darwin files
